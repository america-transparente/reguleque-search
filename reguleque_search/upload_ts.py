...
def import_chunk(chunk, api_key, endpoint, session, port=443, protocol="https"):
    ...
    with open(chunk, "rb") as f:
        with tqdm(total=file_size, unit="B", unit_scale=True, unit_divisor=1024) as t:
            ...
            return session.post(
                f"{protocol}://{endpoint}:{port}/collections/{collection_name}/documents/import?action=upsert",
                data=wrapped_file,
                headers={
...
def main(
    ...
    api_key: Union[str, None] = os.getenv("TYPESENSE_API_KEY"),
    drop: bool = False,
        ...
        for filepath in filepaths:
            ...
            for chunk in chunks:
                ...
                if error_ratio >= warn_error_tolerance:
                    typer.echo(
                        LOG_WARN
                        + f" {round(error_ratio * 100, 1)}% of entries had errors. Sample:",
                        err=True,
                    )
                    print(errors[1])
